...
#Penalty weight to calculate population merit
c_w<-20
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    sapply(X = reps, FUN = function(r) {
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        theta_w<-mean(G[select.id, select.id]) 
        #Calculate the population merit
        B_w<-g_w-c_w*theta_w
        #Initiate B_w_old
        B_w_old<-0
        ...
        while (B_w!=B_w_old){
          ...
          for (i in seq(parents.sel.intensity)){
            ...
            for(j in seq(length(available.id))){
              ...
              #Calculate the average genetic relationship of the parental population
              theta_w_temp<-mean(G[select.temp, select.temp])
              
              #Calculate the new population merit
              B_w_temp<-g_w_temp-c_w*theta_w_temp
              
              #If the replacement of the i-th parent with the j-th individual increases the population merit, the parents are replaced 
              if (B_w_temp>B_w){
                #Update the population merit
                B_w<-B_w_temp
  ...
  filename <- file.path(save.dir, paste("simulation_results_Population_Merit", "_c_",c_w ,"_rep_", n.iterations,"_", date, ".RData", sep = "") )
