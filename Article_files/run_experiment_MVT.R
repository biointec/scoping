...
n.iterations = 5#Genome$data$n.iterations
...
for (change in tp.change) {
  ...
  experiment.sub.results <- mclapply(X = iters.per.core, FUN = function(iter.set) {
    ...
    sapply(X = reps, FUN = function(r) {
      ...
      for (breeding.cycle in seq(n.cycles)) {
        ...
        #Pre-select the top-300
        sort.id<-order(predictions.out$GEBV, decreasing=T)[1:300]
        
        #Use the GEBVs to guide the parental selection (pre-selection of 300)
        value.mat = predictions.out$GEBV[sort.id[seq(1,300)],]
        l.names <- names(value.mat) #Line names
        
        #First parental selection of the P2
        select.id<-seq(51,100)
        
        #Keep track of the selected and available individuals
        id=seq(length(value.mat))
        available.mask<-matrix(T,length(value.mat),1)
        available.mask[c(seq(50),select.id)]=F
        ...
        #Calculate frequency of second allele of the current breeding cycle (before breeding)
        ...
        #Calculate averaged genetic relationship of parental population
        theta_w<-mean(G[c(seq(50),select.id), c(seq(50),select.id)])
        
        var_sum<-0
        for (i in seq(parents.sel.intensity/2)){
          var_sum<-var_sum+((1+F_inb[select.id[i]])-2*theta_w)
          var_sum<-var_sum+((1+F_inb[seq(50)[i]])-2*theta_w)
        }
        #Calculate the genetic variance
        sigma_G<-as.numeric(var_sum/parents.sel.intensity)
        
        #Initiate sigma_G_old
        sigma_G_old<-0
        
        #Maximize sigma_G
        while (sigma_G!=sigma_G_old){
          sigma_G_old=sigma_G
          ...
          for (i in seq(parents.sel.intensity/2)){
            ...
            for(j in seq(length(available.id))){
              ...
              #Calculate averaged genetic relationship of parental population
              theta_w_temp<-mean(G[c(seq(50), select.temp), c(seq(50) , select.temp)])
              
              var_sum<-0
              for (k in seq(parents.sel.intensity/2)){
                var_sum<-var_sum+((1+F_inb[select.temp[k]])-2*theta_w_temp)
                var_sum<-var_sum+((1+F_inb[seq(50)[k]])-2*theta_w_temp)
              }
              
              #Calculate the genetic variance
              sigma_G_temp<-as.numeric(var_sum/parents.sel.intensity)
              
              #If the replacement of the i-th parent with the j-th individual increases the MVT, the parents are replaced 
              if (sigma_G_temp>sigma_G){
                #Update the MVT
                sigma_G<-sigma_G_temp
        ...
        sel.val<-as.matrix(value.mat[c(seq(50),select.id)])
        parent.selections.i<-list(value.sel=sel.val ,lines.sel=rownames(sel.val))
       
        parent.lines.list <- list(p1 = parent.selections.i$lines.sel[seq(50)], 
                                  p2 = parent.selections.i$lines.sel[seq(51,100)])
  ...
  filename <- file.path(save.dir, paste("simulation_results_max_var","_rep_", n.iterations,"_", date, ".RData", sep = "") )
